...
class GitHubHelper:
    ...
    def get_repo_details(self, owner, repo_name):
        response = requests.get(f"{self.BASE_URL}/repos/{owner}/{repo_name}", headers=self.headers)
        if response.status_code == 200:
            return response.json()
        return None

    def get_repo_languages(self, owner, repo_name):
        response = requests.get(f"{self.BASE_URL}/repos/{owner}/{repo_name}/languages", headers=self.headers)
        if response.status_code == 200:
            return response.json()
        return {}
    ...
    def analyze_repository(self, owner, repo_name):
        ...
        # Doc Score (Check for README, size of description)
        doc_score = 0
        if repo_data.get("description"): doc_score += 30
        if repo_data.get("has_wiki"): doc_score += 20
        if repo_data.get("has_pages"): doc_score += 20
        if repo_data.get("size", 0) > 100: doc_score += 30
        
        # Maint Score (Last push date)
        from datetime import datetime
        last_push = datetime.strptime(repo_data["pushed_at"], "%Y-%m-%dT%H:%M:%SZ")
        now = datetime.now()
        days_since_push = (now - last_push).days
        maint_score = max(0, 100 - (days_since_push / 10)) # Lose 1 point every 10 days
        
        # Overall Score
        overall_score = round((doc_score + maint_score + (min(repo_data.get("stargazers_count", 0), 100))) / 3)
